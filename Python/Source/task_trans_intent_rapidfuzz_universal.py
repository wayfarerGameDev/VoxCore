...
class TaskTransIntentRapidFuzz:
    command = "--task_trans_intent_rapidfuzz_universal"
    ...
    def boot(self):
        self.route_command("--ui_header Task booted: Trans_Intent_Rapidfuzz_Universal")
    
    def terminate(self):
        self.route_command("--ui_header Task terminated: Trans_Intent_Rapidfuzz_Universal")
        self._rules.clear()
    ...
    def on_command(self, text: str) -> bool:
        ...
        if text.startswith("--intent_add "):
            ...
            if sentence:
                ...
                self._parse_rules()
                self.route_command(f"--ui_debug Trans_Intent_Rapidfuzz_Universal: Intent added -> '{sentence}'")
            return False
        ...
        if text.startswith("--intent_remove "):
            ...
            if sentence in self._rules:
                ...
                self._parse_rules()
                self.route_command(f"--ui_debug Trans_Intent_Rapidfuzz_Universal: Intent removed -> '{sentence}'")
            return False
